...
export async function crs(f:FastifyInstance){
  f.get('/transform', async (req, reply)=>{
    const { coords, from='EPSG:4326', to='EPSG:3857' } = (req.query as any)||{}
    if(!coords) return reply.code(400).send({error:'coords required'})
